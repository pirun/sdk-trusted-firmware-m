...
def process_manifest(manifest_list_files, extra_manifests_list):
    """
    ...
    for manifest_item in manifest_list:
        ...
        if OUT_DIR is not None:
            if 'output_path' in manifest_item:
                ...
                outfile_name = os.path.join(output_path, "psa_manifest", outfile_name).replace('\\', '/')
                intermediafile_name = os.path.join(output_path, "auto_generated", 'intermedia_' + context['file_name'] + '.c').replace('\\', '/')
                infofile_name = os.path.join(output_path, "auto_generated", 'load_info_' + context['file_name'] + '.c').replace('\\', '/')
            else:
...
def main():
    ...
    partition_db = process_manifest(manifest_list, extra_manifests_list)
